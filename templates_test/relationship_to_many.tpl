...
func test{{$txt.LocalTable.NameGo}}ToMany{{$txt.Function.Name | IDFK}}(t *testing.T) {
	var err error
	...
	{{$varname}}, err := a.{{$txt.Function.Name | IDFK}}(tx).All()
	if err != nil {
		t.Fatal(err)
	}
	...
	if err = a.L.Load{{$txt.Function.Name | IDFK}}(tx, false, &slice); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.{{$txt.Function.Name | IDFK}}); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
	}

	a.R.{{$txt.Function.Name | IDFK}} = nil
	if err = a.L.Load{{$txt.Function.Name | IDFK}}(tx, true, &a); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.{{$txt.Function.Name | IDFK}}); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
