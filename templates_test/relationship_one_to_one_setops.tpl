...
func test{{$txt.LocalTable.NameGo}}OneToOneSetOp{{$txt.ForeignTable.NameGo}}Using{{$txt.Function.Name | IDFK}}(t *testing.T) {
	var err error
	...
	for i, x := range []*{{$txt.ForeignTable.NameGo}}{&b, &c} {
		err = a.Set{{$txt.Function.Name | IDFK}}(tx, i != 0, x)
		if err != nil {
			t.Fatal(err)
		}

		if a.R.{{$txt.Function.Name | IDFK}} != x {
			t.Error("relationship struct not set to correct value")
		}
		if x.R.{{$txt.Function.ForeignName | IDFK}} != &a {
...
func test{{$txt.LocalTable.NameGo}}OneToOneRemoveOp{{$txt.ForeignTable.NameGo}}Using{{$txt.Function.Name | IDFK}}(t *testing.T) {
	var err error
	...
	if err = a.Set{{$txt.Function.Name | IDFK}}(tx, true, &b); err != nil {
		t.Fatal(err)
	}

	if err = a.Remove{{$txt.Function.Name | IDFK}}(tx, &b); err != nil {
	...
	if a.R.{{$txt.Function.Name | IDFK}} != nil {
