...
func test{{$txt.LocalTable.NameGo}}OneToOne{{$txt.ForeignTable.NameGo}}Using{{$txt.Function.Name | IDFK}}(t *testing.T) {
	...
	check, err := local.{{$txt.Function.Name | IDFK}}(tx).One()
	if err != nil {
		t.Fatal(err)
	}
	...
	if err = local.L.Load{{$txt.Function.Name | IDFK}}(tx, false, &slice); err != nil {
		t.Fatal(err)
	}
	if local.R.{{$txt.Function.Name | IDFK}} == nil {
		t.Error("struct should have been eager loaded")
	}

	local.R.{{$txt.Function.Name | IDFK}} = nil
	if err = local.L.Load{{$txt.Function.Name | IDFK}}(tx, true, &local); err != nil {
		t.Fatal(err)
	}
	if local.R.{{$txt.Function.Name | IDFK}} == nil {
		t.Error("struct should have been eager loaded")
